# app.py
import os
import logging
from io import BytesIO
from datetime import datetime, date

from flask import Flask, request, abort
from telegram import Bot, Update, InputFile
from telegram.ext import Dispatcher, CommandHandler, MessageHandler, Filters
import openpyxl
import xlrd

logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)s %(message)s")
logger = logging.getLogger(__name__)

# ------------------------------------------
# Config / webhook URL
# ------------------------------------------
TOKEN = os.environ.get("TOKEN")
if not TOKEN:
    logger.error("TOKEN environment variable not set. Set TOKEN in Render environment.")
    raise SystemExit("TOKEN not set")

SERVICE_URL = os.environ.get("RENDER_EXTERNAL_URL") or "https://telegram-excel-bot-11.onrender.com"

WEBHOOK_PATH = f"/webhook/{TOKEN}"
WEBHOOK_URL = f"{SERVICE_URL.rstrip('/')}{WEBHOOK_PATH}"

# Flask app
app = Flask(__name__)

# Telegram Bot + Dispatcher
bot = Bot(token=TOKEN)
dispatcher = Dispatcher(bot, None, workers=0, use_context=True)

# ------------------------------------------
# Session state machine
# ------------------------------------------
S_WAITING_FILE = "WAITING_FILE"
S_WAITING_COLUMN = "WAITING_COLUMN"
S_WAITING_SUBSTRING = "WAITING_SUBSTRING"
S_WAITING_PICK = "WAITING_PICK"

sessions = {}

def init_session(chat_id):
    sessions[chat_id] = {
        "state": S_WAITING_FILE,
        "file_name": None,
        "headers": None,
        "rows": None,
        "chosen_col": None,
        "candidates": None,
    }
    return sessions[chat_id]

def get_session(chat_id):
    return sessions.get(chat_id) or init_session(chat_id)

def cleanup_session(chat_id):
    sessions.pop(chat_id, None)

def reset_session(chat_id, bot_obj):
    cleanup_session(chat_id)
    init_session(chat_id)
    bot_obj.send_message(chat_id, "Send a new Excel file to begin.")

# ------------------------------------------
# Excel utilities
# ------------------------------------------
def stringify_cell(val):
    if val is None:
        return ""
    if isinstance(val, (datetime, date)):
        return val.strftime("%d-%m-%Y")
    if isinstance(val, float):
        if val.is_integer():
            return str(int(val))
        return format(val, "f").rstrip("0").rstrip(".")
    if isinstance(val, int):
        return str(val)
    return str(val).strip()

def load_excel_clean(file_bytes: bytes, file_name: str):
    ext = file_name.lower()
    rows = []
    headers = []

    if ext.endswith(".xlsx"):
        wb = openpyxl.load_workbook(BytesIO(file_bytes), data_only=True)
        ws = wb.active
        rows_iter = list(ws.iter_rows(values_only=True))
        if not rows_iter:
            return [], []
        headers = [stringify_cell(v) or f"Column {i+1}" for i, v in enumerate(rows_iter[0])]
        for r in rows_iter[1:]:
            rows.append([stringify_cell(v) for v in r])
        return rows, headers

    if ext.endswith(".xls"):
        try:
            book = xlrd.open_workbook(file_contents=file_bytes)
            sheet = book.sheet_by_index(0)
            if sheet.nrows == 0:
                return [], []
            headers = [stringify_cell(sheet.cell_value(0, c)) or f"Column {c+1}" for c in range(sheet.ncols)]
            for r in range(1, sheet.nrows):
                rows.append([stringify_cell(sheet.cell_value(r, c)) for c in range(sheet.ncols)])
            return rows, headers
        except xlrd.biffh.XLRDError as e:
            msg = str(e).lower()
            if "xlsx file; not supported" in msg or "expected xls workbook" in msg:
                wb = openpyxl.load_workbook(BytesIO(file_bytes), data_only=True)
                ws = wb.active
                rows_iter = list(ws.iter_rows(values_only=True))
                if not rows_iter:
                    return [], []
                headers = [stringify_cell(v) or f"Column {i+1}" for i, v in enumerate(rows_iter[0])]
                for r in rows_iter[1:]:
                    rows.append([stringify_cell(v) for v in r])
                return rows, headers
            raise

    raise ValueError("Unsupported file type. Use .xls or .xlsx")

def build_output_workbook(header_row, rows_to_write):
    wb = openpyxl.Workbook()
    ws = wb.active
    ws.title = "Filtered"

    ws.append(header_row)
    for row in rows_to_write:
        if len(row) < len(header_row):
            row = row + [""] * (len(header_row) - len(row))
        ws.append(row)

    for col_idx in range(1, ws.max_column + 1):
        max_len = 0
        for r in range(1, ws.max_row + 1):
            val = ws.cell(r, col_idx).value
            if val:
                max_len = max(max_len, len(str(val)))
        ws.column_dimensions[openpyxl.utils.get_column_letter(col_idx)].width = min(max(8, max_len + 2), 50)

    bio = BytesIO()
    wb.save(bio)
    bio.seek(0)
    return bio

# ------------------------------------------
# Handlers
# ------------------------------------------
def start(update, context):
    reset_session(update.effective_chat.id, context.bot)

def document_handler(update, context):
    chat_id = update.effective_chat.id
    sess = get_session(chat_id)
    file_name = update.message.document.file_name or "file.xlsx"

    try:
        tgfile = context.bot.get_file(update.message.document.file_id)
        bio = BytesIO()
        tgfile.download(out=bio)
        bio.seek(0)

        rows, headers = load_excel_clean(bio.read(), file_name)
        if not headers:
            context.bot.send_message(chat_id, "Could not read file. Send another file.")
            return reset_session(chat_id, context.bot)

        sess["rows"] = rows
        sess["headers"] = headers
        sess["file_name"] = file_name
        sess["state"] = S_WAITING_COLUMN

        lines = [f"{i+1}. {h}" for i, h in enumerate(headers)]
        context.bot.send_message(chat_id,
                                 f"File loaded. {len(headers)} columns found:\n\n" +
                                 "\n".join(lines) +
                                 "\n\nSend column number.")
    except Exception as e:
        context.bot.send_message(chat_id, f"Error reading file: {e}")
        return reset_session(chat_id, context.bot)

def text_handler(update, context):
    chat_id = update.effective_chat.id
    text = update.message.text.strip()
    sess = get_session(chat_id)
    state = sess["state"]

    if text.lower() in ("cancel", "/cancel", "stop"):
        context.bot.send_message(chat_id, "Cancelled.")
        return reset_session(chat_id, context.bot)

    if state == S_WAITING_FILE:
        context.bot.send_message(chat_id, "Send an Excel file first.")
        return

    if state == S_WAITING_COLUMN:
        try:
            col = int(text)
            if not (1 <= col <= len(sess["headers"])):
                context.bot.send_message(chat_id, "Invalid column number.")
                return
            sess["chosen_col"] = col - 1
            sess["state"] = S_WAITING_SUBSTRING
            context.bot.send_message(chat_id, "Send substring to search.")
        except:
            context.bot.send_message(chat_id, "Enter a valid column number.")
        return

    if state == S_WAITING_SUBSTRING:
        substr = text.lower()
        col = sess["chosen_col"]
        matches = [r[col].strip() for r in sess["rows"]
                   if substr in r[col].strip().lower()]

        distinct = list(dict.fromkeys(matches))

        if not distinct:
            context.bot.send_message(chat_id, f"No matches for '{text}'.")
            return reset_session(chat_id, context.bot)

        if len(distinct) == 1:
            context.bot.send_message(chat_id, f"Found: {distinct[0]}. Filtering...")
            do_filter_and_send(context.bot, chat_id, sess, distinct[0])
            return reset_session(chat_id, context.bot)

        sess["candidates"] = distinct
        sess["state"] = S_WAITING_PICK
        lines = [f"{i+1}. {v}" for i, v in enumerate(distinct)]
        context.bot.send_message(chat_id, "Multiple matches:\n" + "\n".join(lines) + "\nSend number.")
        return

    if state == S_WAITING_PICK:
        try:
            idx = int(text) - 1
            cand = sess["candidates"]
            if not (0 <= idx < len(cand)):
                context.bot.send_message(chat_id, "Invalid choice.")
                return
            chosen = cand[idx]
            context.bot.send_message(chat_id, f"Selected: {chosen}. Filtering...")
            do_filter_and_send(context.bot, chat_id, sess, chosen)
            return reset_session(chat_id, context.bot)
        except:
            context.bot.send_message(chat_id, "Send valid number.")
            return

    context.bot.send_message(chat_id, "Invalid input.")
    return reset_session(chat_id, context.bot)

def do_filter_and_send(bot_obj, chat_id, sess, chosen_value):
    col = sess["chosen_col"]
    header = sess["headers"]
    rows = sess["rows"]

    filtered = [r for r in rows if r[col].strip().lower() == chosen_value.lower()]

    if not filtered:
        bot_obj.send_message(chat_id, f"No rows found for '{chosen_value}'.")
        return

    bio = build_output_workbook(header, filtered)
    out_name = f"{os.path.splitext(sess['file_name'])[0]}_filtered.xlsx"
    bio.seek(0)

    bot_obj.send_document(chat_id, InputFile(bio, filename=out_name),
                          caption=f"{len(filtered)} rows found.")

# ------------------------------------------
# Webhook route
# ------------------------------------------
@app.route(WEBHOOK_PATH, methods=["POST"])
def webhook_route():
    data = request.get_json(force=True)
    update = Update.de_json(data, bot)
    dispatcher.process_update(update)
    return "OK"

# ------------------------------------------
# Webhook setup
# ------------------------------------------
def set_webhook():
    try:
        ok = bot.set_webhook(WEBHOOK_URL)
        logger.info("Webhook set: %s", ok)
    except Exception:
        logger.exception("Failed to set webhook")

if __name__ == "__main__":
    set_webhook()
    port = int(os.environ.get("PORT", 10000))
    app.run(host="0.0.0.0", port=port)
